...
import '../application/targets_bloc.dart';
...
class TargetsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<TargetsBloc, TargetsState>(
        listener: (BuildContext context, TargetsState state) {
        ...
        builder: (BuildContext context, TargetsState state) {
          ...
          final TargetsLoaded? loadedState =
              state is TargetsLoaded ? state : null;
          final List<Target> trainingTargets =
              loadedState?.trainingTargets ?? <Target>[];
          final List<Target> macroTargets =
              loadedState?.macroTargets ?? <Target>[];
          final List<String> availableMuscles =
              _getAvailableMuscleGroups(trainingTargets);
                        ...
                        child: ListView(
                          ...
                          children: <Widget>[
                            ...
                            if (trainingTargets.isEmpty)
                              ...
                              ...trainingTargets.map(
                                (Target target) => _buildTrainingTargetCard(
                                  context,
                                  target,
                                ),
                              ),
                            ...
                            if (macroTargets.isEmpty)
                              ...
                              ...macroTargets.map(
                                (Target target) => _buildMacroTargetCard(
                                  context,
                                  target,
                                ),
                              ),
  ...
  Widget _buildTrainingTargetCard(BuildContext context, Target target) {
    final String displayName = MuscleGroups.getDisplayName(target.categoryKey);
  ...
  Widget _buildMacroTargetCard(BuildContext context, Target target) {
    final String displayName = _macroDisplayName(target.categoryKey);
  ...
  List<String> _getAvailableMuscleGroups(List<Target> trainingTargets) {
    final Set<String> targetedMuscles =
        trainingTargets.map((Target t) => t.categoryKey).toSet();
    return MuscleGroups.all
        .where((String muscle) => !targetedMuscles.contains(muscle))
        .toList();
  }
...
class _TrainingTargetDialogState extends State<_TrainingTargetDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final List<String> selectableMuscles = _isEditing
        ? <String>[_selectedMuscle!]
      ...
      content: Column(
        ...
        children: <Widget>[
          ...
          DropdownButtonFormField<String>(
            ...
            items: selectableMuscles.map((String muscle) {
...
class _MacroTargetDialogState extends State<_MacroTargetDialog> {
  ...
  void _submit() {
    final double? parsedValue = double.tryParse(_valueController.text.trim());
