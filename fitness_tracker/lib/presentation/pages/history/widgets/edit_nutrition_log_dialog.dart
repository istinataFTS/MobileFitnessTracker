...
class _EditNutritionLogDialogState extends State<EditNutritionLogDialog> {
  ...
  Widget _buildMealEditor(BuildContext context) {
    return BlocBuilder<MealBloc, MealState>(
      builder: (context, state) {
        ...
        Meal? meal;
        try {
          meal = state.meals.firstWhere((item) => item.id == widget.log.mealId);
        } catch (_) {
          meal = null;
        }
  ...
  void _saveMealLog(BuildContext context) {
    ...
    if (mealState is! MealsLoaded) return;

    Meal? meal;
    try {
      meal = mealState.meals.firstWhere((item) => item.id == widget.log.mealId);
    } catch (_) {
      meal = null;
    }
