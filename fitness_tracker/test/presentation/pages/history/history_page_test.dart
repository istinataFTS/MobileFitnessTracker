...
import 'package:fitness_tracker/presentation/pages/history/history_page.dart';
...
void main() {
  ...
  final januaryMonth = DateTime(2024, 1, 1);
  ...
  setUp(() {
    ...
    when(() => exerciseBloc.state).thenReturn(
      ExercisesLoaded([benchPress]),
    );
    whenListen(
      exerciseBloc,
      const Stream<ExerciseState>.empty(),
      initialState: ExercisesLoaded([benchPress]),
    );
  });
  ...
  group('HistoryPage', () {
    ...
    testWidgets('shows loaded state', (tester) async {
      final state = HistoryLoaded(
        ...
        monthSets: {
          selectedDate: [workoutSet],
        },
        monthNutritionLogs: {
          selectedDate: [nutritionLog],
        },
        selectedDate: selectedDate,
        selectedDateSets: [workoutSet],
        selectedDateNutritionLogs: [nutritionLog],
      );
    ...
    testWidgets('tapping a calendar date dispatches SelectDateEvent',
      ...
      final state = HistoryLoaded(
        ...
        selectedDateNutritionLogs: const [],
      );
